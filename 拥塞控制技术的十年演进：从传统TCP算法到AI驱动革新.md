...
​	在过去十年里，互联网流量与应用场景的多样化促使拥塞控制技术不断演进。本文首先回顾了传统基于丢包反馈的 TCP 拥塞控制算法（如 Reno、CUBIC）的设计与局限，继而分析了 Google BBR 算法提出的模型驱动思路，以及数据中心内部 RDMA 环境下 DCQCN 与阿里巴巴 HPCC 算法的显式信号闭环控制机制。近年来，人工智能技术的兴起催生了对超低延迟与高带宽的苛刻诉求，促使研究者将分布式网络遥测（INT）与强化学习（如 Meta ACC）的自适应策略相结合。实验结果表明，各阶段算法均在带宽利用率与延迟抑制之间进行了不同权衡，并在各自应用场景中取得了显著性能提升。最后，本文展望了 6G 与算力网络时代下的自适应、智能化拥塞控制方向，指出未来应聚焦多维度网络状态感知与跨域协同优化。

## 引言

​	随着云计算、大数据与移动视频等应用的高速发展，全球互联网流量在过去十年中以数十倍的速度攀升。超大规模视频分发、分布式 AI 训练和实时交互游戏等场景对网络带宽和延迟提出了前所未有的挑战：一方面，需要尽可能高效地利用现有链路容量以提升吞吐率；另一方面，又要严格控制端到端时延，避免“Bufferbloat”及突发丢包对服务质量的破坏。2016年之前，以CUBIC和Reno为代表的基于丢包反馈的TCP算法在高带宽、高延迟网络中表现不佳，无法有效应对视频流媒体、云计算等应用对网络性能的严苛要求。这些经典算法不仅在高带宽延迟积（BDP）环境下效率低下，还面临着与其他算法共存时的公平性问题。尽管早期出现了如DCTCP等改进方案，但其适用范围主要局限于数据中心短距离流量，难以满足更广泛的网络需求。

​	传统基于丢包反馈的 TCP 拥塞控制算法（如 Reno、CUBIC）在早期互联网架构中凭借 AIMD 原则取得了稳定且公平的带宽分配，但在高带宽-延迟积（BDP）或大规模数据中心网络中，其收敛速度慢、易引发持久排队，已难以满足现代应用需求。为此，研究界先后提出了以 Google BBR 为代表的模型驱动方法，通过主动测量瓶颈带宽和最小 RTT，突破了丢包触发的被动局限；在数据中心内部，DCQCN 和 HPCC 利用 ECN 明确信号和可编程遥测技术，构建了端到端的闭环流量调节系统；近两年，随着机器学习与在线遥测的发展，ACC 等 AI 驱动策略将自适应阈值调整和强化学习引入拥塞控制，开启了网络智能化的全新篇章。  

​	本论文旨在系统梳理过去十年拥塞控制技术的演进脉络，深入剖析各代算法的核心设计思路与实现细节，并结合典型实验评估结果，比较它们在不同场景下的性能表现。最后，我们将展望未来 6G 和算力网络时代下的自适应、跨层协同拥塞控制方向，以期为下一代高性能网络提供参考。
...
​	本文对过去十年拥塞控制技术的发展历程进行了系统梳理与评估。  

1. 传统丢包反馈算法（Reno、CUBIC）以加法增／乘法减（AIMD）原则为核心，奠定了 TCP 生态的基础，但在高 BDP 和大缓冲设备环境下暴露出 Bufferbloat 及收敛速度慢的问题。  
2. BBR 算法基于带宽–延迟产品模型，通过显式测量瓶颈带宽与最小 RTT，实现了“以带宽驱动、以时延为戒”的控制思路，大幅提升了长流吞吐率并抑制了持续排队。  
3. 针对数据中心无损网络场景，DCQCN 与 HPCC 分别从交换机 ECN 信号和可编程硬件遥测两方面构建了闭环控制系统，在 RDMA 训练与微秒级延迟需求下展现了卓越性能。  
4. 最近期的研究以 ACC 等 AI 驱动方法为代表，将在线智能优化与传统信号闭环相结合，实现了对 ECN 标记阈值与发包节奏的自动调节，进一步推动了网络性能与应用灵活性的融合。  

​	展望未来，6G 和算力网络将带来更高带宽、更严苛的端到端时延要求，网络状态智能感知与跨层协同优化将成为关键。我们相信，通过融合分布式遥测、机器学习与网络协议设计的深度协同，下一代拥塞控制技术将在海量连接和实时计算任务中创造更稳定、高效的网络体验。
