...
​	采用快速排序算法的框架，但是在 $partition$ 时用线性时间选择算法选择中位数作为 $pivot$ ，递归至数组被分为 $k$ 段时结束算法。由于线性时间选择算法的时间复杂度为 $\Omicron(n)$ ，所以每次 $partition$ 的时间复杂度仍为 $\Omicron(n)$ ，共进行 $\log{k}$ 次  。因此算法的时间复杂度为 $\Omicron(n\log{k})$ 。
