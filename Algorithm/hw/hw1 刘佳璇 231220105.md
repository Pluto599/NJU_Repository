...
​	最坏情况和平均情况的时间复杂度均为 $T(n)=\sum\limits_{i=n}^2\sum\limits_{j=1}^{i-1}1=\sum\limits_{i=2}^n(i-1)=\frac{n(n-1)}{2}=\Theta(n^2).$  
...
​	对某一给定的 $i,\;\forall\;k\,(\,j<k<i\,),\;A[k]\le A[i]$ 为循环不变式。

​	当 $i=1$ 时，$j=0,\;k$ 不存在，循环不变式成立。

​	在一次循环中，若当前的 $j>0 且 A[j]\le A[i]$，则 $j=P[j]$。$A[P[j]]$ 到 $A[j]$ 之间的元素均小于 $A[i]$ 。直至 $j=0$ 或 $A[j]>A[i]$，循环结束，循环不变式仍成立。
...
```c++
isSub(a,b): return (a subscribes b)
isCel[1..n]={true};
for i=1 to n do
 |  if isCel[i]=true then
 |   |   for j=1 to n and j!=i do
 |   |   |  if isSub(i,j) then
 |   |   |   |_ isSub[i]=false;
 |	 |   |	else
 |_  |_  |_  |_ isSub[j]=false;
for i=1 to n do
 |  if isCel[i]=true then
 |_  |_ return i;
return 0;              
