...
​	先用各个链表的表头元素构建 $k$ 个元素的最小堆，弹出堆顶后用堆顶所属链表的下一个元素替换堆顶；重复操作直至所有链表为空；然后依次弹出堆顶直至堆空。构建堆的时间复杂度为 $\Omicron(k)$ ，修复最小堆的时间复杂度为 $\Omicron(\log{k})$ ，共修复 $n$ 次，故算法的时间复杂度为 $\Omicron(k+n\log{k})=\Omicron(n\log{k})$ 。

### 14.6 动态发现中值

​	假设数组大小为 $n$ ，维护一个最小堆 $H_{min}$ 用于存储前 $\lceil\dfrac{n}{2}\rceil$ 大的数，和一个最大堆 $H_{max}$ 用于存储后 $\lfloor\dfrac{n}{2}\rfloor$ 大的数。

​	插入元素 $x$ ：

​		若 $H_{min}.size=H_{max}.size$ 且  $x<H_{min}.top$ ，则先 $H_{max}.push(x)$ ，然后弹出 $H_{max}.top$ 并将其插入 $H_{min}$ ；

​		若 $H_{min}.size=H_{max}.size$ 且  $x\ge H_{min}.top$ ，则 $H_{min}.push(x)$ ；

​		若  $H_{min}.size>H_{max}.size$ 且  $x<H_{min}.top$ ，则 $H_{max}.push(x)$ 。

​		若  $H_{min}.size>H_{max}.size$ 且  $x\ge H_{min}.top$ ，则先 $H_{min}.push(x)$ ，然后弹出 $H_{min}.top$ 并将其插入 $H_{max}$ 。

​	删除中位数：

​		若 $H_{min}.size=H_{max}.size$ ，则先弹出 $H_{min}.top$ ，然后然后弹出 $H_{max}.top$ 并将其插入 $H_{min}$ ；

​		若  $H_{min}.size>H_{max}.size$，则弹出 $H_{min}.top$ 。

​	查找中位数：

​		若 $H_{min}.size=H_{max}.size$ ，则取 $\dfrac{H_{min}.top+H_{max}.top}{2}$ ；

​		若  $H_{min}.size>H_{max}.size$，则取 $H_{min}.top$ 。