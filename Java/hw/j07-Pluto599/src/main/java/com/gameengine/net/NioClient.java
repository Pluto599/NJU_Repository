        ...
        Thread t = new Thread(() -> {
            try {
                while (channel.isOpen()) {
                    ...
                    int shootX = 0;
                    int shootY = 0;
                    if (input.isKeyPressed(73)) shootY -= 1; // I
                    if (input.isKeyPressed(74)) shootX -= 1; // J
                    if (input.isKeyPressed(75)) shootY += 1; // K
                    if (input.isKeyPressed(76)) shootX += 1; // L
                    int shootFlag = (shootX != 0 || shootY != 0) ? 1 : 0;
                    float sx = 0f;
                    float sy = 0f;
                    if (shootFlag == 1) {
                        double mag = Math.max(1e-6, Math.hypot(shootX, shootY));
                        sx = (float) (shootX / mag);
                        sy = (float) (shootY / mag);
                    }
                    String line = "INPUT:" + vx + "," + vy + "," + sx + "," + sy + "," + shootFlag + "\n";
