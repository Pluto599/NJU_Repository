...
public class Renderer extends JFrame {
    ...
    private void initialize() {
        ...
        setResizable(false);
        setUndecorated(true); // 移除窗口边框实现全屏效果
     ...
     * 绘制文本（水平居中）。
     *
     * @param centerX 文本中心点的 X 坐标
     * @param y       文本基线（baseline）的 Y 坐标
     */
    public void drawTextCentered(String text, float centerX, float y, float size, float r, float g, float b, float a) {
        gamePanel.addDrawable(new TextDrawable(text, centerX, y, size, r, g, b, a, true));
    }
    ...
    private static class RectDrawable implements Drawable {
        ...
        public RectDrawable(float x, float y, float width, float height, float r, float g, float b, float a) {
            ...
            this.color = new Color(
                    Math.min(Math.max((int) (r * 255), 0), 255),
                    Math.min(Math.max((int) (g * 255), 0), 255),
                    Math.min(Math.max((int) (b * 255), 0), 255),
                    Math.min(Math.max((int) (a * 255), 0), 255));
        }
    ...
    private static class TextDrawable implements Drawable {
        ...
        public TextDrawable(String text, float x, float y, float size, float r, float g, float b, float a) {
            this(text, x, y, size, r, g, b, a, false);
        }
        ...
        public void draw(Graphics2D g) {
            g.setColor(color);
            // 使用中文字体
            Font newFont = new Font("Microsoft YaHei", Font.BOLD, (int) size);
