...
public class Player extends GameObject {
	...
	public static Player getInstance(String name, Scene scene, Renderer renderer) {
		if (instance == null) {
			instance = new Player(name, scene, renderer);
		}
		return instance;
	}
    ...
    private Player(String name, Scene scene, Renderer renderer) {
        super(name);
        this.scene = scene;
		...
		// 添加变换组件
		TransformComponent transform = this.addComponent(new TransformComponent(new Vector2(400, 300)));
