...
public class PhysicsComponent extends Component<PhysicsComponent> {
    ...
    public void update(float deltaTime) {
        ...
        // 应用摩擦力
        Vector2 friction_acceleration = velocity.normalize().multiply(-friction / mass);
        // 将摩擦加速度添加到总加速度中
        Vector2 velocity_after_friction = velocity.add(friction_acceleration.multiply(deltaTime));
        // 防止速度反向
        if (velocity.dot(velocity_after_friction) < 0) {
    ...
    public void setFriction(float friction) {
        this.friction = Math.max(0, Math.min(500, friction));
    }
