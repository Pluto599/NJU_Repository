...
#define TCP_MAX_BACKLOG 128
    ...
    // TODO 3.2
    // used to send probe packets at regular intervals
    struct tcp_timer persist_timer; // 持续定时器
    // 线程同步等待
    ...
    // 发包时：在tcp_send_packet调用之前上锁，之后解锁。这里由于每个人实现不一样，加锁位置不一定一样，比如在调tcp_send_packet之前，用tcp_tx_window_test检查发送窗口，那么应该在tcp_tx_window_test之前上锁。
