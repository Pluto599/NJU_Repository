...
{
    log(DEBUG, "tcp_sock_close called, current state: %s", tcp_state_str[tsk->state]);
    // fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    if (tsk->state == TCP_ESTABLISHED)
    {
        while (!list_empty(&tsk->send_buf))
            ;
        log(DEBUG, "tcp_sock_close client: send FIN");
        tcp_send_control_packet(tsk, TCP_FIN);
        tcp_set_state(tsk, TCP_FIN_WAIT_1);
    }
    else if (tsk->state == TCP_CLOSE_WAIT)
    {
        log(DEBUG, "tcp_sock_close server: send FIN");
        tcp_send_control_packet(tsk, TCP_FIN);
