    ...
    switch (tsk->state)
    {
        ...
        case TCP_ESTABLISHED:
            ...
            if ((cb->flags & TCP_ACK) && !(cb->flags & TCP_FIN) && cb->pl_len == 0)
            {
                ...
                log(DEBUG, "ack's num is %u", cb->ack);
                tcp_update_window_safe(tsk, cb);
                tcp_update_send_buffer(tsk, cb->ack);
        ...
        case TCP_FIN_WAIT_1:
            log(DEBUG, "tcp_process in TCP_FIN_WAIT_1 state");
            if (cb->flags & TCP_ACK)
            {
                log(DEBUG, "tcp_process in TCP_FIN_WAIT_1 state receive ACK");
        ...
        case TCP_FIN_WAIT_2:
            if (cb->flags & TCP_FIN)
            {
                log(DEBUG, "tcp_process in TCP_FIN_WAIT_2 state receive FIN");
...
int tcp_recv_ofo_buffer_add_packet(struct tcp_sock *tsk, struct tcp_cb *cb)
{
    ...
    log(DEBUG, "tcp_recv_ofo_buffer_add_packet: seq = %u, seq_end = %u", cb->seq - tsk->iss, cb->seq_end - tsk->iss);
