...
void tcp_scan_timer_list()
{
    ...
    list_for_each_entry_safe(timer, tmp, &timer_list, list)
    {
        if (timer->enable)
        {
            ...
            if(timer->type == 1)
            {
                log(DEBUG, "tcp_scan_timer_list: retrans timer: %d", timer->timeout);
            }
            ...
            switch (timer->type)
            {
                case 0: // timewait
                    if (timer->timeout <= 0)
                    {
                        ...
                        log(DEBUG, "tcp_scan_timer_list: timewait timer expired, releasing tcp sock ");
                        tcp_unset_timewait_timer(tsk);
                        
                        if (tsk->state == TCP_TIME_WAIT)
                        {
                            log(DEBUG, "tcp_scan_timer_list: timewait timer expired, set state to TCP_CLOSED");
                ...
                case 1: // retrans
                    // log(DEBUG, "-------------------------retrans timer: %d", timer->timeout);
                    if (timer->timeout <= 0)
                    {
...
void tcp_set_retrans_timer(struct tcp_sock* tsk)
{
    ...
    else
    {
        ...
        log(DEBUG, "retrans timer added to list");
        tsk->ref_cnt++;
    }
