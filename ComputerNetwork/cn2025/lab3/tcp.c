...
static int copy_flag_str(u8 flags, int flag, char* buf, int start, const char* str, int len)
{
    if (flags & flag)
    {
...
void handle_tcp_packet(char* packet, struct iphdr* ip, struct tcphdr* tcp)
{
    ...
    pthread_mutex_lock(&tsk->sk_lock); // add lock
    tcp_process(tsk, &cb, packet);
    pthread_mutex_unlock(&tsk->sk_lock); // remove lock
}
