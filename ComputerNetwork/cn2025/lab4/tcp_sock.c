#include "tcp_sock.h"
#include "ip.h"
#include "log.h"
#include "rtable.h"
#include "tcp.h"
#include "tcp_hash.h"
#include "tcp_timer.h"

const char *tcp_congestion_state_str[] = {
    "SLOW_START",
    "CONGESTION_AVOIDANCE",
    "FAST_RECOVERY"
};

// TCP socks should be hashed into table for later lookup: Those which
// occupy a port (either by *bind* or *connect*) should be hashed into
// bind_table, those which listen for incoming connection request should be
// hashed into listen_table, and those of established connections should
// be hashed into established_table.

struct tcp_hash_table tcp_sock_table;
#define tcp_established_sock_table tcp_sock_table.established_table
#define tcp_listen_sock_table      tcp_sock_table.listen_table
#define tcp_bind_sock_table        tcp_sock_table.bind_table

inline void tcp_set_state(struct tcp_sock* tsk, int state)
{
    log(DEBUG,
        IP_FMT ":%hu switch state, from %s to %s.",
        HOST_IP_FMT_STR(tsk->sk_sip),
        tsk->sk_sport,
        tcp_state_str[tsk->state],
        tcp_state_str[state]);
    tsk->state = state;
}

// init tcp hash table and tcp timer
void init_tcp_stack()
{
    for (int i = 0; i < TCP_HASH_SIZE; i++)
        init_list_head(&tcp_established_sock_table[i]);

    for (int i = 0; i < TCP_HASH_SIZE; i++)
        init_list_head(&tcp_listen_sock_table[i]);

    for (int i = 0; i < TCP_HASH_SIZE; i++)
        init_list_head(&tcp_bind_sock_table[i]);

    pthread_t timer;
    pthread_create(&timer, NULL, tcp_timer_thread, NULL);
}

// allocate tcp sock, and initialize all the variables that can be determined
// now
struct tcp_sock* alloc_tcp_sock()
{
    struct tcp_sock* tsk = malloc(sizeof(struct tcp_sock));

    memset(tsk, 0, sizeof(struct tcp_sock));

    tsk->state = TCP_CLOSED;
    tsk->rcv_wnd = TCP_DEFAULT_WINDOW;

    init_list_head(&tsk->list);
    init_list_head(&tsk->listen_queue);
    init_list_head(&tsk->accept_queue);
    init_list_head(&tsk->send_buf);
    init_list_head(&tsk->rcv_ofo_buf);

    tsk->rcv_buf = alloc_ring_buffer(tsk->rcv_wnd);

    tsk->wait_connect = alloc_wait_struct();
    tsk->wait_accept = alloc_wait_struct();
    // TODO 3.3
    tsk->wait_recv = alloc_wait_struct();
    tsk->wait_send = alloc_wait_struct();

    // TODO 3.1.1
    // 初始化锁
    pthread_mutex_init(&tsk->sk_lock, NULL);
    pthread_mutex_init(&tsk->rcv_buf_lock, NULL);
    pthread_mutex_init(&tsk->send_buf_lock, NULL);

    // TODO 3.2
    // 初始化各个定时器链表
    init_list_head(&tsk->timewait.list);
    init_list_head(&tsk->retrans_timer.list);
    init_list_head(&tsk->persist_timer.list);

    // 将定时器设置为禁用状态
    tsk->timewait.enable = 0;
    tsk->retrans_timer.enable = 0;
    tsk->persist_timer.enable = 0;

    // 初始化重传计数
    tsk->retrans_timer.retrans_cnt = 0;

    // TODO 4
    // 初始化拥塞控制参数
    tsk->cwnd = TCP_MSS;     
    tsk->ssthresh = 64 * TCP_MSS;  
    tsk->c_state = TCP_SLOW_START; 
    tsk->dup_ack_cnt = 0;       

    return tsk;
}
// TODO 2.1.1 done
// release all the resources of tcp sock
//
// To make the stack run safely,
// each time the tcp sock is refered (e.g. hashed), the ref_cnt is increased by 1.
// each time free_tcp_sock is called, the ref_cnt is decreased by 1, and release the resources practically if ref_cnt is
// decreased to zero.
void free_tcp_sock(struct tcp_sock* tsk)
{
    // fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);

    tsk->ref_cnt--;

    if (tsk->ref_cnt <= 0)
    {
        wait_exit(tsk->wait_connect);
        wait_exit(tsk->wait_accept);
        wait_exit(tsk->wait_recv);
        wait_exit(tsk->wait_send);
        free_wait_struct(tsk->wait_connect);
        free_wait_struct(tsk->wait_accept);
        free_wait_struct(tsk->wait_recv);
        free_wait_struct(tsk->wait_send);

        free_ring_buffer(tsk->rcv_buf);

        tcp_unhash(tsk);
        tcp_bind_unhash(tsk);

        tcp_unset_retrans_timer(tsk);

        free(tsk);
    }
}

// lookup tcp sock in established_table with key (saddr, daddr, sport, dport)
struct tcp_sock* tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
    // fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    struct tcp_sock* tsk;

    int hash = tcp_hash_function(saddr, daddr, sport, dport);
    struct list_head* list = &tcp_sock_table.established_table[hash];

    list_for_each_entry(tsk, list, hash_list)
    {
        if (tsk->sk_sip == saddr && tsk->sk_dip == daddr && tsk->sk_sport == sport && tsk->sk_dport == dport)
        {
            return tsk;
        }
    }

    return NULL;
}

// lookup tcp sock in listen_table with key (sport)
//
// In accordance with BSD socket, saddr is in the argument list, but never used.
struct tcp_sock* tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
    // fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    struct tcp_sock* tsk;

    int hash = tcp_hash_function(0, 0, sport, 0);
    struct list_head* list = &tcp_listen_sock_table[hash];

    list_for_each_entry(tsk, list, hash_list)
    {
        if (tsk->sk_sport == sport)
        {
            return tsk;
        }
    }

    return NULL;
}

// lookup tcp sock in both established_table and listen_table
struct tcp_sock* tcp_sock_lookup(struct tcp_cb* cb)
{
    u32 saddr = cb->daddr, daddr = cb->saddr;
    u16 sport = cb->dport, dport = cb->sport;

    struct tcp_sock* tsk = tcp_sock_lookup_established(saddr, daddr, sport, dport);
    if (!tsk)
        tsk = tcp_sock_lookup_listen(saddr, sport);

    return tsk;
}

// hash tcp sock into bind_table, using sport as the key
static int tcp_bind_hash(struct tcp_sock* tsk)
{
    int bind_hash_value = tcp_hash_function(0, 0, tsk->sk_sport, 0);
    struct list_head* list = &tcp_bind_sock_table[bind_hash_value];
    list_add_head(&tsk->bind_hash_list, list);

    tsk->ref_cnt += 1;

    return 0;
}

// unhash the tcp sock from bind_table
void tcp_bind_unhash(struct tcp_sock* tsk)
{
    if (!list_empty(&tsk->bind_hash_list))
    {
        list_delete_entry(&tsk->bind_hash_list);
        free_tcp_sock(tsk);
    }
}

// lookup bind_table to check whether sport is in use
static int tcp_port_in_use(u16 sport)
{
    int value = tcp_hash_function(0, 0, sport, 0);
    struct list_head* list = &tcp_bind_sock_table[value];
    struct tcp_sock* tsk;
    list_for_each_entry(tsk, list, bind_hash_list)
    {
        if (tsk->sk_sport == sport)
            return 1;
    }

    return 0;
}

// find a free port by looking up bind_table
static u16 tcp_get_port()
{
    for (u16 port = PORT_MIN; port < PORT_MAX; port++)
    {
        if (!tcp_port_in_use(port))
            return port;
    }

    return 0;
}

// tcp sock tries to use port as its source port
static int tcp_sock_set_sport(struct tcp_sock* tsk, u16 port)
{
    if ((port && tcp_port_in_use(port)) || (!port && !(port = tcp_get_port())))
        return -1;

    tsk->sk_sport = port;

    tcp_bind_hash(tsk);

    return 0;
}

// hash tcp sock into either established_table or listen_table according to its TCP_STATE
int tcp_hash(struct tcp_sock* tsk)
{
    struct list_head* list;
    int hash;

    if (tsk->state == TCP_CLOSED)
        return -1;

    if (tsk->state == TCP_LISTEN)
    {
        hash = tcp_hash_function(0, 0, tsk->sk_sport, 0);
        list = &tcp_listen_sock_table[hash];
    }
    else
    {
        int hash = tcp_hash_function(tsk->sk_sip, tsk->sk_dip, tsk->sk_sport, tsk->sk_dport);
        list = &tcp_established_sock_table[hash];

        struct tcp_sock* tmp;
        list_for_each_entry(tmp, list, hash_list)
        {
            if (tsk->sk_sip == tmp->sk_sip && tsk->sk_dip == tmp->sk_dip && tsk->sk_sport == tmp->sk_sport
                && tsk->sk_dport == tmp->sk_dport)
                return -1;
        }
    }

    list_add_head(&tsk->hash_list, list);
    tsk->ref_cnt += 1;

    return 0;
}

// unhash tcp sock from established_table or listen_table
void tcp_unhash(struct tcp_sock* tsk)
{
    if (!list_empty(&tsk->hash_list))
    {
        list_delete_entry(&tsk->hash_list);
        free_tcp_sock(tsk);
    }
}

// XXX: skaddr here contains network-order variables
int tcp_sock_bind(struct tcp_sock* tsk, struct sock_addr* skaddr)
{
    int err = 0;

    // omit the ip address, and only bind the port
    err = tcp_sock_set_sport(tsk, ntohs(skaddr->port));

    return err;
}

// TODO 2.1.2 done
// connect to the remote tcp sock specified by skaddr
//
// XXX: skaddr here contains network-order variables
// 1. initialize the four key tuple (sip, sport, dip, dport);
// 2. hash the tcp sock into bind_table;
// 3. send SYN packet, switch to TCP_SYN_SENT state, wait for the incoming SYN packet by sleep on wait_connect;
// 4. if the SYN packet of the peer arrives, this function is notified, which means the connection is established.
int tcp_sock_connect(struct tcp_sock* tsk, struct sock_addr* skaddr)
{
    // fprintf(stdout, "TODO:  implement %s please.\n", __FUNCTION__);

    tsk->sk_dip = ntohl(skaddr->ip);
    tsk->sk_dport = ntohs(skaddr->port);

    rt_entry_t* rt = longest_prefix_match(tsk->sk_dip);
    if (!rt)
    {
        log(ERROR, "No route to host." IP_FMT, HOST_IP_FMT_STR(tsk->sk_dip));

        return -1;
    }
    tsk->sk_sip = rt->iface->ip;

    if (!tsk->sk_sport)
    {
        int err = tcp_sock_set_sport(tsk, 0);
        if (err)
        {
            log(ERROR, "Failed to allocate a source port.");

            return err;
        }
    }


    tsk->snd_nxt = tcp_new_iss();
    tsk->iss = tsk->snd_nxt;

    tcp_send_control_packet(tsk, TCP_SYN);
    tcp_set_state(tsk, TCP_SYN_SENT);

    tcp_hash(tsk);
    // tcp_set_retrans_timer(tsk);

    int ret = sleep_on(tsk->wait_connect);
    if (ret < 0)
    {
        log(ERROR, "TCP connection timeout.");
        tcp_set_state(tsk, TCP_CLOSED);
        tcp_unhash(tsk);
        return -1;
    }

    log(DEBUG, "TCP connection established.");

    return 0;
}

// set backlog (the maximum number of pending connection requst), switch the TCP_STATE,
// and hash the tcp sock into listen_table
int tcp_sock_listen(struct tcp_sock* tsk, int backlog)
{
    // fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    tsk->backlog = backlog;

    tcp_set_state(tsk, TCP_LISTEN);
    int err = tcp_hash(tsk);
    if (err)
    {
        log(ERROR, "Failed to hash the tcp sock into listen_table.");

        return err;
    }

    return 0;
}

// check whether the accept queue is full
inline int tcp_sock_accept_queue_full(struct tcp_sock* tsk)
{
    if (tsk->accept_backlog >= tsk->backlog)
    {
        log(ERROR, "tcp accept queue (%d) is full.", tsk->accept_backlog);
        return 1;
    }

    return 0;
}

// push the tcp sock into accept_queue
inline void tcp_sock_accept_enqueue(struct tcp_sock* tsk)
{
    if (!list_empty(&tsk->list))
        list_delete_entry(&tsk->list);
    list_add_tail(&tsk->list, &tsk->parent->accept_queue);
    tsk->parent->accept_backlog += 1;
}

// pop the first tcp sock of the accept_queue
inline struct tcp_sock* tcp_sock_accept_dequeue(struct tcp_sock* tsk)
{
    struct tcp_sock* new_tsk = list_entry(tsk->accept_queue.next, struct tcp_sock, list);
    list_delete_entry(&new_tsk->list);
    init_list_head(&new_tsk->list);
    tsk->accept_backlog -= 1;

    return new_tsk;
}

// if accept_queue is not emtpy, pop the first tcp sock and accept it,
// otherwise, sleep on the wait_accept for the incoming connection requests
struct tcp_sock* tcp_sock_accept(struct tcp_sock* tsk)
{
    // fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    while (1)
    {
        if (!list_empty(&tsk->accept_queue))
        {
            struct tcp_sock* ready = tcp_sock_accept_dequeue(tsk);
            return ready;
        }
        else
        {
            int wake = sleep_on(tsk->wait_accept);
            if (wake < 0)
            {
                return NULL;
            }
        }
    }
}

// close the tcp sock, by releasing the resources, sending FIN/RST packet to the peer, switching TCP_STATE to closed
void tcp_sock_close(struct tcp_sock* tsk)
{
    // fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    pthread_mutex_lock(&tsk->sk_lock);
    if (tsk->state == TCP_ESTABLISHED)
    {
        tcp_send_control_packet(tsk, TCP_FIN);
        tcp_set_state(tsk, TCP_FIN_WAIT_1);
    }
    else if (tsk->state == TCP_CLOSE_WAIT)
    {
        tcp_send_control_packet(tsk, TCP_FIN);
        tcp_set_state(tsk, TCP_LAST_ACK);
    }
    else
    {
        tcp_set_state(tsk, TCP_CLOSED);
        tcp_unhash(tsk);
        tcp_bind_unhash(tsk);
    }
    pthread_mutex_unlock(&tsk->sk_lock);

    tcp_unset_retrans_timer(tsk);
    tcp_unset_persist_timer(tsk);

    wait_exit(tsk->wait_connect);
    wait_exit(tsk->wait_accept);
    wait_exit(tsk->wait_recv);
    wait_exit(tsk->wait_send);

    pthread_mutex_lock(&tsk->send_buf_lock);
    struct send_buffer_entry *entry, *q;
    list_for_each_entry_safe(entry, q, &tsk->send_buf, list)
    {
        list_delete_entry(&entry->list);
        free(entry->packet);
        free(entry);
    }
    pthread_mutex_unlock(&tsk->send_buf_lock);

    pthread_mutex_lock(&tsk->rcv_buf_lock);
    struct recv_ofo_buf_entry *ofo_entry, *ofo_q;
    list_for_each_entry_safe(ofo_entry, ofo_q, &tsk->rcv_ofo_buf, list)
    {
        list_delete_entry(&ofo_entry->list);
        free(ofo_entry->data);
        free(ofo_entry);
    }

    if (tsk->rcv_buf)
    {
        free_ring_buffer(tsk->rcv_buf);
        tsk->rcv_buf = NULL;
    }

    pthread_mutex_unlock(&tsk->rcv_buf_lock);
}


// TODO 2.2.1 done
static pthread_mutex_t tcp_sock_read_lock = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t tcp_sock_write_lock = PTHREAD_MUTEX_INITIALIZER;

// 返回值：0表示读到流结尾，对方关闭连接；-1表示出现错误；正值表示读取的数据长度
int tcp_sock_read(struct tcp_sock* tsk, char* buf, int len)
{
    // 检查连接状态
    if (tsk->state != TCP_ESTABLISHED && tsk->state != TCP_CLOSE_WAIT)
    {
        log(ERROR, "tcp_sock_read: connection not established");
        return -1;
    }

    pthread_mutex_lock(&tcp_sock_read_lock);

    while (ring_buffer_empty(tsk->rcv_buf))
    {
        if (tsk->state == TCP_CLOSE_WAIT || tsk->state == TCP_CLOSED || tsk->state == TCP_FIN_WAIT_1
            || tsk->state == TCP_FIN_WAIT_2 || tsk->state == TCP_TIME_WAIT)
        {
            return 0;
        }

        pthread_mutex_unlock(&tcp_sock_read_lock);
        // log(DEBUG, "tcp_sock_read sleep_on");
        sleep_on(tsk->wait_recv);
        // log(DEBUG, "tcp_sock_read wake_up");
        pthread_mutex_lock(&tcp_sock_read_lock);
    }

    int read_len = read_ring_buffer(tsk->rcv_buf, buf, len);

    // 更新前的接收窗口大小
    int pre_rcv_wnd = tsk->rcv_wnd;

    // 正常接收缓冲区占用的空间
    int buf_occupied = ring_buffer_used(tsk->rcv_buf);

    // 乱序缓冲区占用的空间
    int ofo_occupied = 0;
    struct recv_ofo_buf_entry *entry;
    list_for_each_entry(entry, &tsk->rcv_ofo_buf, list)
    {
        ofo_occupied += entry->len;
    }

    // 更新接收窗口
    tsk->rcv_wnd = TCP_DEFAULT_WINDOW - buf_occupied - ofo_occupied;
    log(DEBUG, "tcp_sock_read: rcv_wnd = %u", tsk->rcv_wnd);

    // zero window update message
    if(pre_rcv_wnd < TCP_MSS && tsk->rcv_wnd > TCP_MSS)
    {
        log(DEBUG, "tcp_sock_read: rcv_wnd > TCP_MSS, send ACK");
        tcp_send_control_packet(tsk, TCP_ACK);
    }

    // log(DEBUG, "tcp_sock_read read %d bytes", read_len);

    pthread_mutex_unlock(&tcp_sock_read_lock);

    return read_len;
}

// 返回值：-1表示出现错误；0表示窗口已满；正值表示写入的数据长度
int tcp_sock_write(struct tcp_sock* tsk, char* buf, int len)
{
    // 检查连接状态
    if (tsk->state != TCP_ESTABLISHED)
    {
        log(ERROR, "tcp_sock_write: connection not established");
        return -1;
    }

    pthread_mutex_lock(&tcp_sock_write_lock);

    while (!tcp_tx_window_test(tsk))
    {
        log(DEBUG, "tcp_sock_write: window is full, wait for send");
        pthread_mutex_unlock(&tcp_sock_write_lock);
        
        log(DEBUG,"tcp_sock_write sleep on wait_send");

        sleep_on(tsk->wait_send);
        
        log(DEBUG,"tcp_sock_write wake up from wait_send");
        log(DEBUG, "current useable window size: %u", tsk->snd_una + tsk->snd_wnd - tsk->snd_nxt);

        pthread_mutex_lock(&tcp_sock_write_lock);

        // 重新检查窗口大小
        if (!tcp_tx_window_test(tsk))
        {
            log(DEBUG, "tcp_sock_write: window still full after wakeup");
            // 继续循环等待
        }
    }

    if (tsk->state == TCP_ESTABLISHED)
    {
        log(DEBUG, "tcp_sock_write write %d bytes", len);
        log(DEBUG, "current useable window size: %u", tsk->snd_una + tsk->snd_wnd - tsk->snd_nxt);

        tcp_send_data(tsk, buf, len);

        log(DEBUG,"window size after sending: %u", tsk->snd_una + tsk->snd_wnd - tsk->snd_nxt);

        pthread_mutex_unlock(&tcp_sock_write_lock);
        return len;
    }
    else
    {
        pthread_mutex_unlock(&tcp_sock_write_lock);
        return -1;
    }
}

// TODO 3.1.2 done
// 检查当前的窗口是否能继续发送报文，即空余窗口是否至少为TCP_MSS（TCP Max Segment Size）大小
// 使用tsk->snd_una, tsk->snd_wnd, tsk->snd_nxt计算剩余窗口大小，如果大于TCP_MSS，则返回1，否则返回0
int tcp_tx_window_test(struct tcp_sock *tsk)
{
    u32 snd_remaining = tsk->snd_una + tsk->snd_wnd - tsk->snd_nxt;
    return greater_than_32b(snd_remaining, TCP_MSS);
}

// TODO 4
// 记录拥塞窗口变化的线程函数
void *tcp_cwnd_thread(void *arg)
{
    struct tcp_sock *tsk = (struct tcp_sock *)arg;
    FILE *fp = fopen("tcp_cwnd.csv", "w");
    if (!fp)
    {
        log(ERROR, "无法创建tcp_cwnd.csv文件");
        return NULL;
    }

    // 写入CSV文件头
    fprintf(fp, "time_ms,cwnd_mss,ssthresh_mss,reno_state,event\n");

    // 初始状态记录
    fprintf(fp, "0,%u,%u,%s,%s\n",
            tsk->cwnd / TCP_MSS,
            tsk->ssthresh / TCP_MSS,
            tcp_congestion_state_str[tsk->c_state],
            "receive ack in TCP_RENO_SLOW_START");

    int time_us = 0;
    char full_event[256];

    while (tsk->state == TCP_ESTABLISHED && time_us < 60000000)
    {
        usleep(500); // 每500us记录一次
        time_us += 500;

        // 只在状态或cwnd/ssthresh变化时记录
        pthread_mutex_lock(&tsk->sk_lock);

        static u32 last_cwnd = 0;
        static u32 last_ssthresh = 0;
        static int last_state = -1;

        if (last_cwnd != tsk->cwnd || last_ssthresh != tsk->ssthresh ||
            last_state != tsk->c_state || strlen(tsk->last_event) > 0)
        {
            int time_ms = time_us / 1000;

            if (strlen(tsk->last_event) > 0)
            {
                if (strstr(tsk->last_event, "TCP_RENO_") != NULL)
                {
                    // 已经有完整前缀
                    strcpy(full_event, tsk->last_event);
                }
                else
                {
                    // 需要添加状态
                    sprintf(full_event, "%s%s", tsk->last_event,
                            strstr(tsk->last_event, "TCP_RENO") == NULL ? tcp_congestion_state_str[tsk->c_state] : "");
                }
                // 清空事件
                tsk->last_event[0] = '\0';
            }
            else
            {
                // 默认事件
                sprintf(full_event, "receive ack in TCP_RENO_%s",
                        tcp_congestion_state_str[tsk->c_state]);
            }

            // 写CSV记录
            fprintf(fp, "%d,%u,%u,%s,%s\n",
                    time_ms,
                    tsk->cwnd / TCP_MSS,
                    tsk->ssthresh / TCP_MSS,
                    tcp_congestion_state_str[tsk->c_state],
                    full_event);

            // 更新上次记录的值
            last_cwnd = tsk->cwnd;
            last_ssthresh = tsk->ssthresh;
            last_state = tsk->c_state;
        }

        pthread_mutex_unlock(&tsk->sk_lock);
    }

    fclose(fp);
    return NULL;
}
