...
struct ProcessTable
{
	...
	uint32_t childCount; // Number of child processes
...
#define SEG(type, base, lim, dpl) (SegDesc){((lim) >> 12) & 0xffff, (uint32_t)(base) & 0xffff, \
											((uint32_t)(base) >> 16) & 0xff, type, 1, dpl, 1,  \
											(uint32_t)(lim) >> 28, 0, 0, 1, 1, (uint32_t)(base) >> 24}

#define SEG16(type, base, lim, dpl) (SegDesc){(lim) & 0xffff, (uint32_t)(base) & 0xffff,        \
